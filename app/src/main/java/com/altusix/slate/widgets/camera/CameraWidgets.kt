...
fun updateAllCameraWidgets(context: Context) {
    ...
    val receivers = listOf(CameraPhotoFrameReceiver::class.java, CameraPhotoFrame4x2Receiver::class.java)
...
fun updateCameraWidget(context: Context, appWidgetManager: AppWidgetManager, widgetId: Int) {
    ...
    if (providerClass.contains("4x2")) {
        CameraPhotoFrame4x2Receiver.updatePhotoWidget(context, appWidgetManager, widgetId)
    } else {
        CameraPhotoFrameReceiver.updatePhotoWidget(context, appWidgetManager, widgetId)
    }
}
...
// 2. WIDE PHOTO FRAME (4x2 / Fixed Aspect Photo Display)
